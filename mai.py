def calc_w(matrix: list[list[float]], autocode=False, level=3, crit="") -> tuple[list, list]:
    ...
    for i in range(n):
        ...
        V.append(round(row_product ** (1/n), 3))
        if autocode: print(f") ^ 1/{n} = {round(row_product ** (1/n), 3)}")
    ...
    if autocode:
        ...
        print(f"∑V{crit}i = {" + ".join([str(round(el, 3)) for el in V])} = {sum(V)}")
...
def calc_coherence(matrix: list[list[float]], W: list[float], autocode=False, level=3, crit="") -> float:
    ...
    if autocode:
        ...
        terms = " + ".join([f"Р{i+1}{crit}" for i in range(len(pi))])
        print(f"λmax {crit} = {terms} = {lambdamax}")
...
def main():
    ...
    matrix_K1 = [
        ...
        [1/3, 1/3, 1/2, 1,   1,   2,   3,   4,   5,   6],
        [1/3, 1/3, 1/2, 1,   1,   2,   3,   4,   5,   6],
        [1/4, 1/4, 1/3, 1/2, 1/2, 1,   2,   3,   4,   5],
        [1/5, 1/5, 1/4, 1/3, 1/3, 1/2, 1,   2,   3,   4],
        [1/6, 1/6, 1/5, 1/4, 1/4, 1/3, 1/2, 1,   2,   3],
        [1/7, 1/7, 1/6, 1/5, 1/5, 1/4, 1/3, 1/2, 1,   2],
        [1/8, 1/8, 1/7, 1/6, 1/6, 1/5, 1/4, 1/3, 1/2, 1]
    ]

    matrix_K2 = [
        [1,   1,   2,   2,   3,   3,   4,   5,   6,   7],
        [1,   1,   2,   2,   3,   3,   4,   5,   6,   7],
        [1/2, 1/2, 1,   1,   2,   2,   3,   4,   5,   6],
        [1/2, 1/2, 1,   1,   2,   2,   3,   4,   5,   6],
        [1/3, 1/3, 1/2, 1/2, 1,   1,   2,   3,   4,   5],
        [1/3, 1/3, 1/2, 1/2, 1,   1,   2,   3,   4,   5],
        [1/4, 1/4, 1/3, 1/3, 1/2, 1/2, 1,   2,   3,   4],
        [1/5, 1/5, 1/4, 1/4, 1/3, 1/3, 1/2, 1,   2,   3],
        [1/6, 1/6, 1/5, 1/5, 1/4, 1/4, 1/3, 1/2, 1,   2],
        [1/7, 1/7, 1/6, 1/6, 1/5, 1/5, 1/4, 1/3, 1/2, 1]
    ]

    matrix_K3 = [
        [1,   1,   2,   2,   3,   3,   1/2, 1/2, 1/2, 1],
        [1,   1,   2,   2,   3,   3,   1/2, 1/2, 1/2, 1],
        [1/2, 1/2, 1,   1,   2,   2,   1/3, 1/3, 1/3, 1/2],
        [1/2, 1/2, 1,   1,   2,   2,   1/3, 1/3, 1/3, 1/2],
        [1/3, 1/3, 1/2, 1/2, 1,   1,   1/4, 1/4, 1/4, 1/3],
        [1/3, 1/3, 1/2, 1/2, 1,   1,   1/4, 1/4, 1/4, 1/3],
        [2,   2,   3,   3,   4,   4,   1,   1,   1,   2],
        [2,   2,   3,   3,   4,   4,   1,   1,   1,   2],
        [2,   2,   3,   3,   4,   4,   1,   1,   1,   2],
    ...
    matrix_K4 = [
        ...
        [1,   1,   1,   2,   2,   3,   4,   5,   6,   7],
        [1,   1,   1,   2,   2,   3,   4,   5,   6,   7],
        [1/2, 1/2, 1/2, 1,   1,   2,   3,   4,   5,   6],
        [1/2, 1/2, 1/2, 1,   1,   2,   3,   4,   5,   6],
        [1/3, 1/3, 1/3, 1/2, 1/2, 1,   2,   3,   4,   5],
        [1/4, 1/4, 1/4, 1/3, 1/3, 1/2, 1,   1,   2,   3],
        [1/5, 1/5, 1/5, 1/4, 1/4, 1/3, 1,   1,   2,   3],
        [1/6, 1/6, 1/6, 1/5, 1/5, 1/4, 1/2, 1/2, 1,   2],
        [1/7, 1/7, 1/7, 1/6, 1/6, 1/5, 1/3, 1/3, 1/2, 1]
    ]
    ...
    priorities = list(alternative_synthesis(w2i, w3k_list))
